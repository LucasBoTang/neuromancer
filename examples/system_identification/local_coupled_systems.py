...
class RC_Network_old(Coupled_NonAutonomous):
    def __init__(self, R = None, C = None, U=None, nsim=1001, ninit=0, ts=0.1, adj=None, periods = None, nx=2, seed=59):
        """_summary_
        ...
        self.R = R if R is not None else self.get_R(
            self.adj_list, amax=20, amin=5, symmetric=True)
        self.C = C if C is not None else self.get_C(nx)
        self.U = U if U is not None else self.get_U(nsim, nx, periods)
        self.R_ext = self.get_R(np.tile(np.arange(nx),(2,1)), amax=15, symmetric=False)
        
    def get_U(self, nsim, nx, periods = None):
        if periods is None:
            periods = int(np.ceil(nsim / 2000))
        global_source = (1/2)*(Periodic(nsim=nsim, xmin=270.0, xmax=285.0, numPeriods=2)+Periodic(nsim=nsim, xmin=275.0, xmax=280.0, numPeriods=7))
        #global_source = Periodic(nsim=nsim, xmin=270.0, xmax=285.0, numPeriods=2)
        ind_sources = Periodic(nx=nx, nsim=nsim, form = 'square', numPeriods=periods, xmax=15, xmin = 0)
        return np.hstack([global_source,ind_sources])
    ...
    def equations(self, x, t, u):
        ...
        #Internal heat sources
#        deltas = internal_sources - x # assumed unit mis-match
#        deltas[internal_sources==0] = 0
#        dx += (1.0 / self.C) * deltas
        dx += (1.0 / self.C) * internal_sources
        return dx
